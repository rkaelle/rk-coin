...
#if defined(NDEBUG)
# error "Rkcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xdfc3553142509d6ce4d3c40dda25506dba40733e80687425e2032d20a5865b9e");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Rkcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Rkcoin Signed Message:\n";
...
{
    // Rkcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 42000); // Rkcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // Rkcoin: 3.5 days
static const int64 nTargetSpacing = 2 * 60; // Rkcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Rkcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Rkcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x58f0440af1c5d8451b8e79f63296afc4e182dd93694c4a043834ee1bb43482c0");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "02/08/2022 Faceboook stock crashes more than 25 percent loosing over 200b";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 25 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("043b0b1114d34f399fefe0f42205650f52f57d13f18024e49afb3f567e74cdf9b8378e191b08f69e46030c1d47951b4eb6a9a7b87005f01ac17e65d7802ab8a2a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1644380009;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084919352;

        if (fTestNet)
        {
            block.nTime    = 1644379989;
            block.nNonce   = 386365078;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x4eaa75b5c253f711ebe1db7a1b8acce07085bf585df8946cae84404789791cc5"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf2, 0xc5, 0xb7, 0xd2 }; // Rkcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("RkcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("RkcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static RkcoinMiner(CWallet *pwallet)
{
    printf("RkcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("rkcoin-miner");
    ...
    try { loop {
        ...
        printf("Running RkcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&RkcoinMiner, pwallet));
}
